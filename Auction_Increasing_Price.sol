//INCREASING-PRICE AUCTION - minimum bid amount can be increased by the auction owner

pragma solidity ^0.6.4;

contract AuctionIncreasing {
    ...
    event BidIncrementedEvent(uint256 newBidAmt, uint256 time);
    ...
    //the owner can increase the bid amount via this function, the argument is amount to be increased in wei
    function increaseBid(uint256 _incAmt) external only_owner an_ongoing_auction returns(uint256){
        startBid=startBid+_incAmt;
        emit BidIncrementedEvent(startBid,now);
        return startBid;
    }
